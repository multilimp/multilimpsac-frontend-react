  ...
  return <Box>
      <Stack p={3} spacing={2}>
        <Stack direction="row" alignItems="center" justifyContent="center" spacing={2}>
          <Box component="img" src="/images/multilimp-logo.svg" alt="Logo" height={50} sx={{
          filter: 'brightness(0) invert(1)'
        }} // Make logo white
        />
          <Typography variant="h5" fontWeight={700} sx={{
          color: 'white'
        }}>MULTILIMPSAC</Typography>
        </Stack>
        ...
        }}>{user.rol.toUpperCase()}</FormHelperText>
        </Stack>
      </Stack>
      <Divider sx={{
      ...
      <Box component="nav">
        ...
        p: 2
          ...
          <Stack spacing={2}>
            {sidebarList.map(item => <Stack key={item.title} spacing={1.5}>
                <FormHelperText sx={{
              mb: 1,
              ...
              fontSize: '0.75rem'
                ...
                {item.routes.map(record => <NavItem key={record.path} pathname={pathname} {...record} />)}
              </Stack>)}
          </Stack>
        </Scrollbar>
      </Box>
...
function NavItem({
  ...
  return <Box component={Link} to={path} sx={{
    alignItems: 'center',
    borderRadius: 2,
