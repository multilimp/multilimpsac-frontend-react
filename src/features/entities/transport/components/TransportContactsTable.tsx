...
interface TransportContactsTableProps {
  contacts: TransportContact[];
  isLoading: boolean;
  onEdit: (contact: TransportContact) => void;
  onDelete: (contact: TransportContact) => void;
}

const TransportContactsTable: React.FC<TransportContactsTableProps> = ({
  contacts,
  isLoading,
  onEdit,
  onDelete,
  ...
  if (contacts.length === 0) {
    return (
      <div className="text-center p-4 text-muted-foreground">
        No hay contactos asociados a este transporte.
      </div>
    );
  }
...
export default TransportContactsTable;
